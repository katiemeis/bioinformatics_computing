    ...
    with open(filename, 'r') as handle, open(filename2, 'r') as handle2, open(outfilename, 'w') as outfile:
        ...
        for i in range(1,len(fly_seq)):
            for j in range(1,len(human_seq)):
                val_choices = {}
                val_choices[1] = max(align_table[i,j-1] + gap_val, 0)
                val_choices[2] = max(align_table[i-1,j] + gap_val, 0)
                if fly_seq[i] == human_seq[j]:
                    val_choices[3] = max(align_table[i-1,j-1] + match_val, 0)
                elif fly_seq[i] != human_seq[j]:
                    val_choices[3] = max(align_table[i-1,j-1] + mis_val, 0)
                ...
                if max(val_choices.values()) >= max_val:
                    max_val = max(val_choices.values())
                    max_row = i
                    max_col = j
